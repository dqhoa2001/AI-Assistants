...
class ProjectAnalyzer {
    constructor() {
        ...
        this.projectDescription = document.getElementById(
            "project-description"
        );
    ...
    setupEventListeners() {
        ...
        document
            ...
            ?.addEventListener("click", () => this.updateProjectDetails());
    }
    ...
    async analyzeProject() {
        ...
        try {
            ...
            alert(
                error.message || "An error occurred while analyzing the project"
            );
        } finally {
            this.hideLoading();
        ...
        if (this.editButton) {
            this.editButton.addEventListener("click", () => {
                const selectedSection =
                    this.analysisResults?.querySelector(".selected-section");
    ...
    toggleMarkdownView() {
        ...
        if (isMarkdown) {
            ...
            if (textarea) {
                ...
                // Gửi request update
                this.updateAnalysis(this.originalMarkdown)
                    .then(() => {
                        // Chuyển về dạng HTML sau khi update thành công
                        currentContent.innerHTML = DOMPurify.sanitize(
                            marked.parse(this.originalMarkdown)
                        );
...
\`Code\`</pre>
    ...
    async updateAnalysis(content) {
        if (!this.projectId) {
            throw new Error(
                "Project ID not found. Please try analyzing the project again."
            );
        }
    ...
    async updateProjectDetails() {
        ...
        try {
            const response = await fetch(
                `/integraflow/update/${this.projectId}`,
                {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
            ...
            if (!data.success) {
                throw new Error(
                    data.error || "Failed to update project details"
                );
            }

            // Update the project in the list
            const projectItem = document.querySelector(
                `.project-item[data-project-id="${this.projectId}"]`
            );
            if (projectItem) {
                projectItem.querySelector("h4").textContent = name;
                projectItem.querySelector("p").textContent =
    ...
    checkForChanges() {
        ...
        const hasChanges =
            ...
            this.projectDescription.value.trim() !==
                this.originalProjectData.description;
    ...
    updateProjectsList(projects) {
        ...
        projects.forEach((project) => {
            ...
            projectItem.className =
                "project-item cursor-pointer rounded-lg transition-colors duration-200";
